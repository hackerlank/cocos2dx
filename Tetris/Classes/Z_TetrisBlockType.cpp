...
void Z_TetrisBlockType::init()
{
	...
	if(t_typeIndex == 1 || t_typeIndex == 3)
	{
		t_typeIndex = 1;
	...
	if(t_typeIndex == 0 || t_typeIndex == 2)
	{
		t_typeIndex = 0;
...
bool Z_TetrisBlockType::T_checkDropDown()
{
	if (t_typeIndex == 1)//L方向，以下顺时旋转
	{
...
void Z_TetrisBlockType::T_dropDown()
{
...
bool Z_TetrisBlockType::T_checkLeftMove()
{
	if (t_typeIndex == 1)
	{
		TetrisBlock *_block0 = (TetrisBlock*)t_childBlockArray->objectAtIndex(0);
...
void Z_TetrisBlockType::T_leftMove()
{
	for (unsigned int i = 0; i < t_count; ++i)
	{
		TetrisBlock * _block = (TetrisBlock*)t_childBlockArray->objectAtIndex(i);
...
bool Z_TetrisBlockType::T_checkRightMove()
{
	if (t_typeIndex == 1)
	{
		TetrisBlock *_block2 = (TetrisBlock*)t_childBlockArray->objectAtIndex(2);
...
void Z_TetrisBlockType::T_rightMove()
{
	for (unsigned int i = 0; i < t_count; ++i)
	{
		TetrisBlock *_block = (TetrisBlock*)t_childBlockArray->objectAtIndex(i);
...
void Z_TetrisBlockType::T_rotate()
{
	...
	if (t_typeIndex == 0)//竖向转为横向
	{
		...
		int _index_one = _sharedHandle->h_getIndexAt(_firstBlockColum - 1, _firstBlockRow + 1);//左边的位置
		int _index_second = _sharedHandle->h_getIndexAt(_firstBlockColum, _firstBlockRow + 1);//右边的位置
		int _index_third = _sharedHandle->h_getIndexAt(_firstBlockColum + 1, _firstBlockRow);
		...
		_block2->b_Update_C_R(_firstBlockColum - 1, _firstBlockRow + 1);
	...
	if (t_typeIndex == 1)
	{
		...
		int _index_one = _sharedHandle->h_getIndexAt(_firstBlockColum - 1, _firstBlockRow - 1);
...
void Z_TetrisBlockType::T_updateTetrisStateArray(int _value)
{
	for (unsigned int i = 0; i < t_count; ++i)
	{
		TetrisBlock *_block = (TetrisBlock*)t_childBlockArray->objectAtIndex(i);
