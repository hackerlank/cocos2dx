...
bool L_TetrisBlockType::T_checkDropDown()
{
	...
	if (t_typeIndex == 2)//
	{
		...
		TetrisBlock *_block2 = (TetrisBlock*)t_childBlockArray->objectAtIndex(2);
		if (_block0->b_checkWithDirection(0) && _block1->b_checkWithDirection(0)
			&& _block2->b_checkWithDirection(0))
		{
			return true;
		}
	}
...
void L_TetrisBlockType::T_dropDown()
{
...
bool L_TetrisBlockType::T_checkLeftMove()
{
	if (t_typeIndex == 0)
	{
		TetrisBlock *_block2 = (TetrisBlock*)t_childBlockArray->objectAtIndex(2);
	...
	if (t_typeIndex == 1)
	{
		...
		if (_block0->b_checkWithDirection(1) && _block1->b_checkWithDirection(1) &&
			_block2->b_checkWithDirection(1))
		{
			return true;
		}
	}
...
void L_TetrisBlockType::T_leftMove()
{
	for (unsigned int i = 0; i < t_count; ++i)
	{
		TetrisBlock * _block = (TetrisBlock*)t_childBlockArray->objectAtIndex(i);
...
bool L_TetrisBlockType::T_checkRightMove()
{
	if (t_typeIndex == 0)
	{
		TetrisBlock *_block1 = (TetrisBlock*)t_childBlockArray->objectAtIndex(1);
	...
	if (t_typeIndex == 3)
	{
		...
		TetrisBlock *_block2 = (TetrisBlock*)t_childBlockArray->objectAtIndex(2);
		if (_block0->b_checkWithDirection(2) && _block1->b_checkWithDirection(2) &&
			_block2->b_checkWithDirection(2))
		{
			return true;
		}
	}
...
void L_TetrisBlockType::T_rightMove()
{
	for (unsigned int i = 0; i < t_count; ++i)
	{
		TetrisBlock *_block = (TetrisBlock*)t_childBlockArray->objectAtIndex(i);
...
void L_TetrisBlockType::T_rotate()
{
	...
	if (t_typeIndex == 0)
	{
		...
		int _index_one = _sharedHandle->h_getIndexAt(_firstBlockColum, _firstBlockRow + 1);
		int _index_second = _sharedHandle->h_getIndexAt(_firstBlockColum, _firstBlockRow - 1);
		int _index_third = _sharedHandle->h_getIndexAt(_firstBlockColum + 1, _firstBlockRow - 1);
	...
	if (t_typeIndex == 2)//竖向转为横向
	{
		if (_firstBlockRow - 1 < 0)
		{
			return;
		}

		int _index_one = _sharedHandle->h_getIndexAt(_firstBlockColum, _firstBlockRow - 1);//左边的位置
...
void L_TetrisBlockType::T_updateTetrisStateArray(int _value)
{
	for (unsigned int i = 0; i < t_count; ++i)
	{
		TetrisBlock *_block = (TetrisBlock*)t_childBlockArray->objectAtIndex(i);
